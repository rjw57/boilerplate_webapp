"""
The :py:mod:`boilerplate_webapp` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~boilerplate_webapp.apps.Config` class's
:py:meth:`~boilerplate_webapp.apps.AssetsConfig.ready` method.
    ...
    for idx, name in enumerate(REQUIRED_SETTINGS):
        ...
        if value is None or value == '':
            errors.append(Error(
                ...
                id='boilerplate_webapp.E{:03d}'.format(idx + 1),
